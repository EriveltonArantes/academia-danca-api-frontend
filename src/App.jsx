...
const apiBaseUrl = "https://academia-danca-api.onrender.com";
  ...
  return (
    <div className="login-screen">
      <div className="login-institucional">
        <div className="login-tema-icone">🏋️</div>
        <h1>Academia Danca</h1>
        <p className="login-slogan">Saúde e performance</p>
        <div className="login-quemsomos">
          <h3>Quem somos</h3>
          <p>Sistema de gestão profissional, com controle de acesso por usuário e dado protegido.</p>
        </div>
        <ul className="login-features">
          <li>✓ Controle completo de Professor</li>
          <li>✓ Controle completo de Aluno</li>
          <li>✓ Controle completo de Turma</li>
          <li>✓ Controle completo de Matricula</li>
        </ul>
      </div>
...
function PainelProfessor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [alunoList, setAlunoList] = React.useState([]);
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "cpf", "estilo", "bio", "disponivel", "telefone", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/professors", { headers: authHeaders(token) })
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/alunos", { headers: authHeaders(token) }).then(r => r.json()).then(setAlunoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/professors/" + editId : apiBaseUrl + "/api/professors";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "cpf", "estilo", "bio", "disponivel", "telefone"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/professors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Professor")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>email</b><span>{item["email"] ?? "—"}</span></div>
            <div className="item-field"><b>cpf</b><span>{item["cpf"] ?? "—"}</span></div>
            <div className="item-field"><b>estilo</b><span>{item["estilo"] ?? "—"}</span></div>
            <div className="item-field"><b>bio</b><span>{item["bio"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>aluno</b><span>{item.alunoId ? ((alunoList.find(o => o.id === item.alunoId) || {}).nome ?? ("#" + item.alunoId)) : "—"}</span></div>
            </div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Professor</h3>
      ...
      <select value={form["aluno"] ?? ""} onChange={e => setForm({...form, aluno: e.target.value})}>
        ...
        {(alunoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
...
function PainelAluno({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "cpf", "dataNascimento", "telefone", "nivel", "objetivo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/alunos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/turmas", { headers: authHeaders(token) }).then(r => r.json()).then(setTurmaList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/alunos/" + editId : apiBaseUrl + "/api/alunos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "cpf", "dataNascimento", "telefone", "nivel", "objetivo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["turma"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/alunos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Aluno")}</div>
            ...
            <div className="item-field"><b>objetivo</b><span>{item["objetivo"] ?? "—"}</span></div>
            <div className="item-field"><b>turma</b><span>{item.turmaId ? ((turmaList.find(o => o.id === item.turmaId) || {}).professorId ?? ("#" + item.turmaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Aluno</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <select value={form["turma"] ?? ""} onChange={e => setForm({...form, turma: e.target.value})}>
        <option value="">Selecione...</option>
        {(turmaList || []).map(o => (<option key={o.id} value={o.id}>{o.professorId ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelTurma({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [professorList, setProfessorList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["professorId", "nome", "estilo", "nivel", "diasSemana", "horario", "duracao", "vagas", "mensalidade", "ativa", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/turmas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/professors", { headers: authHeaders(token) }).then(r => r.json()).then(setProfessorList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/turmas/" + editId : apiBaseUrl + "/api/turmas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["professorId", "nome", "estilo", "nivel", "diasSemana", "horario", "duracao", "vagas", "mensalidade", "ativa"].forEach(k => { f[k] = item[k] ?? ""; });
    ["professor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/turmas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["professorId"] ?? "Turma")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>nome</b><span>{item["nome"] ?? "—"}</span></div>
            <div className="item-field"><b>estilo</b><span>{item["estilo"] ?? "—"}</span></div>
            <div className="item-field"><b>nivel</b><span>{item["nivel"] ?? "—"}</span></div>
            <div className="item-field"><b>diasSemana</b><span>{item["diasSemana"] ?? "—"}</span></div>
            <div className="item-field"><b>horario</b><span>{item["horario"] ?? "—"}</span></div>
            <div className="item-field"><b>duracao</b><span>{item["duracao"] ?? "—"}</span></div>
            <div className="item-field"><b>vagas</b><span>{item["vagas"] ?? "—"}</span></div>
            <div className="item-field"><b>mensalidade</b><span>{item["mensalidade"] != null ? Number(item["mensalidade"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>ativa</b><span>{item["ativa"] != null ? (item["ativa"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>professor</b><span>{item.professorId ? ((professorList.find(o => o.id === item.professorId) || {}).nome ?? ("#" + item.professorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Turma</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">professorId</label>
      <input type="text" value={form["professorId"] ?? ""} onChange={e => setForm({...form, professorId: e.target.value})} />
      <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      <label className="field-label">estilo</label>
      <input type="text" value={form["estilo"] ?? ""} onChange={e => setForm({...form, estilo: e.target.value})} />
      ...
      <label className="field-label">diasSemana</label>
      <input type="text" value={form["diasSemana"] ?? ""} onChange={e => setForm({...form, diasSemana: e.target.value})} />
      ...
      <input type="text" value={form["duracao"] ?? ""} onChange={e => setForm({...form, duracao: e.target.value})} />
      <label className="field-label">vagas</label>
      <input type="number" value={form["vagas"] ?? ""} onChange={e => setForm({...form, vagas: e.target.value})} />
      <label className="field-label">mensalidade</label>
      <input type="number" step="0.01" value={form["mensalidade"] ?? ""} onChange={e => setForm({...form, mensalidade: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["ativa"] === true || form["ativa"] === "true"} onChange={e => setForm({...form, ativa: e.target.checked})} />
        ativa
      </label>
      <label className="field-label">professor</label>
      <select value={form["professor"] ?? ""} onChange={e => setForm({...form, professor: e.target.value})}>
        <option value="">Selecione...</option>
        {(professorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelMatricula({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "turmaId", "dataInicio", "status", "mensalidade", "formaPagamento", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/matriculas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/matriculas/" + editId : apiBaseUrl + "/api/matriculas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "turmaId", "dataInicio", "status", "mensalidade", "formaPagamento"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "turma"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/matriculas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "Matricula")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/matriculas/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "Matricula")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>alunoId</b><span>{item["alunoId"] ?? "—"}</span></div>
            <div className="item-field"><b>turmaId</b><span>{item["turmaId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>mensalidade</b><span>{item["mensalidade"] != null ? Number(item["mensalidade"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>formaPagamento</b><span>{item["formaPagamento"] ?? "—"}</span></div>
            <div className="item-field"><b>aluno</b><span>{item.alunoId ? ((alunoList.find(o => o.id === item.alunoId) || {}).nome ?? ("#" + item.alunoId)) : "—"}</span></div>
            <div className="item-field"><b>turma</b><span>{item.turmaId ? ((turmaList.find(o => o.id === item.turmaId) || {}).professorId ?? ("#" + item.turmaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Matricula</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">alunoId</label>
      <input type="number" value={form["alunoId"] ?? ""} onChange={e => setForm({...form, alunoId: e.target.value})} />
      <label className="field-label">turmaId</label>
      <input type="number" value={form["turmaId"] ?? ""} onChange={e => setForm({...form, turmaId: e.target.value})} />
      ...
      <label className="field-label">mensalidade</label>
      <input type="number" step="0.01" value={form["mensalidade"] ?? ""} onChange={e => setForm({...form, mensalidade: e.target.value})} />
      <label className="field-label">formaPagamento</label>
      <select value={form["formaPagamento"] ?? "PIX"} onChange={e => setForm({...form, formaPagamento: e.target.value})}>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Professor");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Academia Danca<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Professor" ? " active" : "")} onClick={() => setAba("Professor")}><span className="nav-ico">🗂️</span>Professor</button>
        <button className={"nav-btn" + (aba === "Aluno" ? " active" : "")} onClick={() => setAba("Aluno")}><span className="nav-ico">👤</span>Aluno</button>
        <button className={"nav-btn" + (aba === "Turma" ? " active" : "")} onClick={() => setAba("Turma")}><span className="nav-ico">📚</span>Turma</button>
        <button className={"nav-btn" + (aba === "Matricula" ? " active" : "")} onClick={() => setAba("Matricula")}><span className="nav-ico">🗂️</span>Matricula</button>
      ...
      <main className="main">
        ...
        {aba === "Professor" && <PainelProfessor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Aluno" && <PainelAluno token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Turma" && <PainelTurma token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Matricula" && <PainelMatricula token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
